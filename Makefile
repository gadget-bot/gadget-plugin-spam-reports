...
EXECUTABLE  := gadget-plugin-spam-reports
...
container: ## Build container using docker
	@$(MAKE) --no-print-directory log-$@
	@docker build -t gadget-plugin-spam-reports:local .
...
start-db: ## Start maria db - export DB_ROOT_PASS and DB_PASS to set credentials
	@$(MAKE) --no-print-directory log-$@
	@docker run --name gadget-plugin-spam-reports-mariadb \
		-v ${HOME}/.gadget-plugin-spam-reports/db:/var/lib/mysql \
		-e MARIADB_ROOT_PASSWORD="${DB_ROOT_PASS}" \
		-e MARIADB_DATABASE=gadget_plugin_spam_reports_dev \
...
stop-db: ## Stop maria db
	@$(MAKE) --no-print-directory log-$@
	@docker stop gadget-plugin-spam-reports-mariadb
	@docker rm gadget-plugin-spam-reports-mariadb
